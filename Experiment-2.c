...
void adc_init(){
	...
	ADCSRA_reg = (ADCSRA_reg | ( (1 << ADEN_bit) | (1 << ADPS2_bit)  | (1 << ADPS1_bit) | (1 << ADIE_bit) )) & ~ (1 << (ADPS0_bit) | (1 << ADSC_bit) | (1 << ADATE_bit) | (1 << ADIF_bit));
	
	// In ADCSRB, disable Analog Comparator Multiplexer, MUX5 bit and ADC Auto Trigger Source bits
	ADCSRB_reg	&= ~( (1 <<MUX5_bit) | (1 << ACME_bit)  | (1 << ADTS2_bit) | (1 << ADTS1_bit) | (1 << ADTS0_bit));
